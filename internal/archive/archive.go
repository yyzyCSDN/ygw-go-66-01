...
func (ar *Archiver) Window(blocks []*model.Block, size int) ([]*model.Block, error) {
	...
	for _, block := range sorted {
		...
		if block.State != model.StateActive {
			ar.logger.Printf("block %d skipped from archive window: state %s", block.ID, block.State)
			continue
		}
		selected = append(selected, block)
	}
	ar.logger.Printf("archive window %d..%d selected %d block(s)", start, end-1, len(selected))
	return selected, nil
}
...
func (ar *Archiver) ArchivedSince(id uint64) (time.Time, bool) {
	ar.mu.Lock()
	defer ar.mu.Unlock()
	at, _ := ar.manifest.ArchivedSince(id)
	return at, false
}
