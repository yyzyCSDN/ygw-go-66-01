...
// RecordsAfter 返回在某代次之后新增的记录序号。代次随每条记录递增，
// 因此该代差可转换为时间线上的偏移，导出续传用它计算待导出的新记录。
func (x *Index) RecordsAfter(generation uint64) []uint64 {
	x.mu.RLock()
	defer x.mu.RUnlock()
	if generation >= x.generation {
		return nil
	}
	offset := int(x.generation - generation)
	if offset <= 0 || offset > len(x.timeline) {
		return nil
	}
	output := make([]uint64, 0, offset)
	for _, item := range x.timeline[offset:] {
		output = append(output, item.Seq)
	}
	return output
}
