...
func (r *Retention) Apply(policy Policy) (Report, error) {
	...
	for _, id := range ids {
		...
		if archivedAt, archived := r.archiver.ArchivedSince(id); archived {
			report.SkippedArchived++
			r.logger.Printf("block %d skipped as archived at %s", id, archivedAt.Format(time.RFC3339))
			continue
		}
...
func (r *Retention) Expired(block *model.Block, policy Policy) bool {
	...
	if block.State != model.StateActive {
		r.logger.Printf("block %d state %s is not expirable", block.ID, block.State)
		return false
	}
	if r.archiver.InActiveWindow(block.ID) {
		r.logger.Printf("block %d sits inside the active archive window", block.ID)
		return false
	}
	cutoff := policy.Cutoff(r.now())
	if !block.CreatedAt.Before(cutoff) {
		r.logger.Printf("block %d created %s stays within retention",
			block.ID, block.CreatedAt.Format(time.RFC3339))
		return false
	}
	return true
}
