...
func (s *Searcher) Page(q index.Query, page, size int) (PageResult, error) {
	...
	total, err := s.index.QueryCount(q)
	if err != nil {
	...
	offset := (page - 1) * size
	if offset >= total {
		return PageResult{Records: []model.Record{}, Total: total, Page: page, Size: size, HasMore: false}, nil
	}
	start, end := pageBounds(len(hits), offset, size)
...
// pageBounds 计算分页区间，确保起始位置不越界且边界记录完整保留。
func pageBounds(hitCount, offset, size int) (int, int) {
	if offset < 0 {
		offset = 0
	}
	end := offset + size
